          ...
          return (
            <Link key={c._id} href={`/clients/${c._id}`} className="block bg-white rounded-xl border border-gray-200 shadow-sm p-4 hover:shadow-md transition-shadow">
              <div className="flex items-start justify-between mb-1">
                ...
                <span className={`px-2 py-0.5 rounded-full text-xs font-semibold capitalize ${c.status === 'active'
              ...
              <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-sm">
                ...
                {showPawnDate && (
                  <div>
                    ...
                    <p className="font-medium">{formatDateIST(c.pawnDate)}</p>
                  </div>
                )}
                {showClosedDate && c.closedDate && (
                  <div>
                    <p className="text-xs text-gray-400">CLOSED ON</p>
                    <p className="font-medium">{formatDateIST(c.closedDate)}</p>
                  </div>
                )}
              </div>
            </Link>
            ...
            {clients.map((c, i) => {
              ...
              return (
                ...
                >
                  ...
                  {showPawnDate && (
                    <td className="px-4 py-3">
                      {formatDateIST(c.pawnDate)}
                    </td>
                  )}
                  {showClosedDate && (
                    <td className="px-4 py-3">
                      {formatDateIST(c.closedDate)}
                    </td>
                  )}
